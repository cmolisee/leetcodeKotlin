...
class IsSubsequenceTest {
    ...
    @ParameterizedTest(name = "{0} should return {1}")
    @MethodSource("args")
    fun `Is Subsequence Recursive`(input: Pair<String, String>, expected: Boolean) {
        assertEquals(expected, isSubsequence.isSubsequenceRecursive(input.first, input.second))
    }
}